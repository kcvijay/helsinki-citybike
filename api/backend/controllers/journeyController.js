...
const getAllData = asyncHandler(async (req, res, collectionName) => {
    let collection;
    ...
    const skip = req.query.offset; // Default offset as 0.
    const data = await collection.find().skip(skip).limit(limit);
    res.status(200).json(data);
});
const getData = asyncHandler(async (req, res, collectionName) => {
    let collection;
    let filter;
    switch (collectionName) {
        case "journeys":
...
const getTopStations = asyncHandler(async (req, res) => {
    const stationId = req.params.stationid;
    try {
