...
const getTopReturnStations = asyncHandler((req: Request, res: Response) => {
  const stationId = req.params.stationid;

  try {
    // Query to get the top return station IDs
    const returnStationIds = await Journey.aggregate([
    ...
    // Extract the return station IDs from the result
    const returnStationIdList = returnStationIds.map((station) => station._id);

    // Query to get the details of the return stations
    const returnStations = await Station.find({
      station_id: { $in: returnStationIdList },
    }).exec();

    // Combine the return station details with the count
    const result = returnStations.map((station) => {
      const count = returnStationIds.find(
        (s) => s._id === station.station_id
      ).count;
      return { ...station.toObject(), count };
    });
