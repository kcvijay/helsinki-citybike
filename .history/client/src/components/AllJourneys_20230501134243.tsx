...
const AllJourneys = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`http://localhost:4000/api/journeys?limit=1`)
      .then((res) => {
        setData(res.data);
        setLoading(false);
      })
  ...
  const totalPages = Math.ceil(Number(data) / Number(inputValue.itemsPerPage));

  const getPageItems = () => {
    const startIndex = (currentPage - 1) * Number(inputValue.itemsPerPage);
    const endIndex = startIndex + Number(inputValue.itemsPerPage);
