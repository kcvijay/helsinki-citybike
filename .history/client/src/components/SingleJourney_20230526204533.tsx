...
const SingleJourney = () => {
  ...
  useEffect(() => {
    if (journeyData && journeyData?.departure_station_id) {
      dispatch(showSingleStation(journeyData.departure_station_id));
    }
  }, [dispatch, journeyData, journeyData?.departure_station_id]);

  useEffect(() => {
    if (stationData) {
      setDepartureStationData(stationData);
    }
  }, [stationData]);

  useEffect(() => {
    if (departureStationData && journeyData?.return_station_id) {
      dispatch(showSingleStation(journeyData.return_station_id));
    }
  }, [dispatch, departureStationData, journeyData?.return_station_id]);

  useEffect(() => {
    if (departureStationData && stationData) {
      setReturnStationData(stationData);
    }
  ...
  useEffect(() => {
    if (
      ...
      const fetchReturnStation = async () => {
        await dispatch(showSingleStation(journeyData.return_station_id));
      };

      fetchReturnStation();
    }
  }, [dispatch, journeyData, departureStationData]);

  useEffect(() => {
    if (journeyData && journeyData.departure_station_id) {
      const fetchDepartureStation = async () => {
        await dispatch(showSingleStation(journeyData.departure_station_id));
      };

      fetchDepartureStation();
    }
  }, [dispatch, journeyData]);

  useEffect(() => {
    if (journeyData && journeyData.return_station_id) {
      const fetchReturnStation = async () => {
        await dispatch(showSingleStation(journeyData.return_station_id));
      };
  ...
  }, [stationData, journeyData?.departure_station_id]);

  useEffect(() => {
    if (journeyData?.return_station_id) {
      setReturnStationData(stationData);
    }
  }, [stationData, journeyData?.return_station_id]);
