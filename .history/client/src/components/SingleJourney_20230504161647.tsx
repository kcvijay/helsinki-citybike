...
const SingleJourney = () => {
  ...
  const [departureStationData, setDepartureStationData] = useState<singleJourneyData>();
  ...
  const handleFetchData = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:4000/api/journeys/${params.stationid}`
      );
      setData(res.data);
    } catch (error: any) {
      alert(error.message);
