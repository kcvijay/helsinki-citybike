...
const AllJourneys = () => {
  ...
  const currentData = data.slice(itemsOffset, Number(endOffset));

  const pageCount = Math.ceil(
    currentData.length / Number(inputValue.itemsPerPage)
  );
  ...
  const clickHandler = (e: any) => {
    const newOffset =
      e.selected * Number(inputValue.itemsPerPage) * Number(itemsOffset.length);
