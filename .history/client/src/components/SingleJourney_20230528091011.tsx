...
const SingleJourney = () => {
  ...
  useEffect(() => {
    const fetchDepartureStationData = async () => {
      const response = await axios.get(
        `https://hkl-citybike.onrender.com/api/stations/${journeyData?.departure_station_id}`
      );
  ...
  useEffect(() => {
    const fetchReturnStationData = async () => {
      const response = await axios.get(
        `https://hkl-citybike.onrender.com/api/stations/${journeyData?.return_station_id}`
      );
      ...
      <div className="wrapper">
        <div className="text-white flex flex-col md:flex-row">
          <div className="w-full mb-6">
            ...
            <h2 className="page-header mb-2 uppercase">
        ...
        <div className="flex justify-start items-center gap-4">
          <p className="flex justify-center items-center bg-white px-4 py-2">
        ...
        <div className="mt-8">
          {departureStationData && returnStationData ? (
            <iframe
              className="w-full min-h-[400px] border-4 border-slate-500"
              title="map"
            ...
            <div className="w-full flex justify-center items-center min-h-[400px] border-4 bg-white border-slate-500">
              <img className="rounded-full" src={loader} alt="Loading icon" />
            </div>
          )}
        </div>
