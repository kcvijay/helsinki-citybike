...
const AllJourneys = () => {
  ...
  const endOffset = itemsOffset + inputValue.itemsPerPage;

  const currentItems = data.slice(itemsOffset, Number(endOffset));

  const pageCount = Math.ceil(data.length / inputValue.itemsPerPage);
  ...
  const clickHandler = (e: any) => {
    const newOffset =
      e.selected * Number(inputValue.itemsPerPage) * itemsOffset.length;
