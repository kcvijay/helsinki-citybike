...
const AllJourneys = () => {
  ...
  // On Page load...
  useEffect(() => {
    handleFetchData(25, 0);
  }, []);

  //On change event
  useEffect(() => {
    if (chosenPageLimit) {
      handleFetchData(chosenPageLimit, 0).then(() => {
  ...
  const handleFetchData = async (perPage?: number, offset?: number) => {
    setLoading(true);
    const limit = perPage === undefined ? "" : perPage;
    const skip = offset === undefined ? "" : offset;
    const res = await axios.get(
      `http://localhost:4000/api/journeys?limit=${limit}&skip=${skip}`
    );
    setData(res.data);
    setLoading(false);
  };
  ...
  const toastify = (items: number) => toast.success(`Showing ${items} items.`);
