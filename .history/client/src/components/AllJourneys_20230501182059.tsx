...
const AllJourneys = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`http://localhost:4000/api/journeys?limit=${1000}&skip=${10}`)
      .then((res) => {
        setData(res.data);
        setLoading(false);
      })
  ...
  const itemsPerPageHandler = (items: string) => {
    ...
    // const limit = inputValue.itemsPerPage;
    axios
      .get(`http://localhost:4000/api/journeys?limit=${limit}&skip=${offset}`)
      .then((res) => {
        setData(res.data);
        setLoading(false);
  ...
  // const endOffset = itemOffset + inputValue.itemsPerPage;
  ...
  const pageCount = Math.ceil(+filteredData.length / inputValue.itemsPerPage);
  const handlePaginationClick = (e: any) => {
    const newOffset =
      (e.selected * inputValue.itemsPerPage) % +filteredData.length;
    console.log(
